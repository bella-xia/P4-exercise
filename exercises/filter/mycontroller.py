...
def writeForwardingRule(p4info_helper, sw, dst_ip_addr, dst_eth_addr, port):
    """
...
def writeFilterRule(p4info_helper, sw, src_ip_addr, src_port):
    ...
    #       program.
    table_entry = p4info_helper.buildTableEntry(
        table_name="MyIngress.filter_match",
        match_fields={"hdr.ipv4.srcAddr": src_ip_addr, "hdr.udp.srcPort": src_port},
...
def writeAllFilterRules(p4info_helper, sw_list):
    ...
    with open("susp.txt") as fp:
        for line in fp:
            ...
            for sw in sw_list:
                writeFilterRule(
                    p4info_helper, sw=sw, src_ip_addr=src_ip_addr, src_port=src_port
                )
